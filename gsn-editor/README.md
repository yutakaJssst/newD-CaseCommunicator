...
- **自動ログイン**: ブラウザを閉じても認証状態を維持
- **ログアウト**: ヘッダー右側のログアウトボタン

**動作確認:**
1. アプリを開くとログイン/登録画面が表示されます
2. 新規登録でアカウント作成（メールアドレス、パスワード必須、名前任意）
3. 登録完了後、自動的にログインしてGSNエディタ画面に遷移
4. ヘッダー右側にユーザー名（またはメールアドレス）とログアウトボタンが表示

**今後の実装予定:**
- **Phase 2**: プロジェクト管理（ユーザーごとに複数のGSNプロジェクトを作成・管理）
- **Phase 3**: マルチユーザー共有（プロジェクトメンバーの招待、権限管理）
- **Phase 4**: リアルタイム同時編集（WebSocketによる複数ユーザーの同時編集）
